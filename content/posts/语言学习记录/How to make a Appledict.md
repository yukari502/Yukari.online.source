...
draft = false
categories = ['语言学习记录']
description = "macOS 的原生词典系统因其深度集成而备受推崇，特别是配合触控板的**三指轻点**取词功能，能够提供无与伦比的便捷查词体验。然而，macOS 自带的词典库虽然质量上乘，但数量和种类有限，难以满足专业或特定语言学习的需求。为了突破这一限制，我们可以通过工具将第三方词典（如 `.mdx` 格式）转换为 macOS 原生支持的格式，实现高度的个性化定制。"
+++
